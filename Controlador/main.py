...
def encriptar(message):
    ...
    key = os.urandom(32) #esto se conecta con el front
    print("La llave privada es: ", key)
...
class BlockChain(list):
    ...
    def transacciones(self):
        return pd.concat(
            [bloque.transacciones for bloque in self],
    ...
    def emitir_Cocoins(self, cantidad):
        if cantidad < -self.bloque_actual.saldos["Banco Central"]:
            ...
            self.bloque_actual.actualizar_transacciones(
                "Imprimiendo nuevos Cocoins", "Banco Central", cantidad
            )
    ...
    def verificar_integridad(self):
        ...
        for bloque in self[-2::-1]:
            ...
            if bloque.código != anterior:
                print(
                    "ADVERTENCIA: LA CADENA DE BLOQUES FUE ADULTERADA EN EL BLOQUE %d!!!"
                    % bloque.índice
                )
                return
            else:
    ...
    def nuevo_desafio(self, p):  # Pos necesito cambiarlo
...
class Transacciones:
    def __init__(self):
        ...
        print(COCOIN.saldos) #banco central
#paga6000 a todos
        for nombre in ["Ana", "Beto", "Carlos", "Diana"]: 
            COCOIN.pagar("Banco Central", nombre, 600)
        print(COCOIN.saldos)
        print(COCOIN.transacciones) #muestra a quien le dio

        np.random.seed(2019)
        NUMERO_DE_BLOQUES = 5

        for i in range(0, NUMERO_DE_BLOQUES):
            COCOIN.crear_siguiente_bloque()  #aqui te pide una clave de 16?
            print("\n", "=" * 60)
            print(f"El código del bloque {COCOIN[-2].índice} es:\n{COCOIN[-2].código}")
            print(
                f"El bloque #{COCOIN.bloque_actual.índice} ha sido agregado a la cadena del Cocoin!"
            )
...
class Red:  # peer to peer con otro pc solana
    def __init__(self, nodos):
        print()


class Ejecucion:  # lo de contratos inteligente con solana
    def __init__(self, consenso):
        print()
